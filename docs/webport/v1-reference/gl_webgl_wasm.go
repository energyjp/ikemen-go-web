...
// Reusable JS-side arena for the hot conversion paths. WebGL entry points
// (uniform*fv, bufferData, texImage2D) copy their input synchronously, so a
// single scratch buffer can back every call on this single-threaded target.
// This avoids allocating two JS objects per draw-call argument, which
// otherwise dominates the garbage produced per frame.
var arenaDisabled = js.Global().Get("location").Truthy() &&
	js.Global().Get("location").Get("search").Truthy() &&
	strings.Contains(js.Global().Get("location").Get("search").String(), "noarena=1")

var (
	scratchU8  js.Value // one persistent Uint8Array
	scratchCap int
	scratchF32 = map[int]js.Value{} // float32 length -> cached view
	scratchU8V = map[int]js.Value{} // byte length -> cached view
)

func scratchEnsure(byteLen int) {
	if byteLen > scratchCap {
		newCap := 1024
		for newCap < byteLen {
			newCap *= 2
		}
		scratchU8 = js.Global().Get("Uint8Array").New(newCap)
		scratchCap = newCap
		scratchF32 = map[int]js.Value{}
		scratchU8V = map[int]js.Value{}
	}
}

func float32sToScratch(s []float32) js.Value {
	if arenaDisabled {
		a := js.Global().Get("Uint8Array").New(len(s) * 4)
		js.CopyBytesToJS(a, sliceToByteSlice(s))
		runtime.KeepAlive(s)
		buf := a.Get("buffer")
		return js.Global().Get("Float32Array").New(buf, 0, len(s))
	}
	scratchEnsure(len(s) * 4)
	js.CopyBytesToJS(scratchU8, sliceToByteSlice(s))
	runtime.KeepAlive(s)
	v, ok := scratchF32[len(s)]
	if !ok {
		v = js.Global().Get("Float32Array").New(scratchU8.Get("buffer"), 0, len(s))
		scratchF32[len(s)] = v
	}
	return v
}

func uint8sToScratch(s []uint8) js.Value {
	if arenaDisabled {
	...
	scratchEnsure(len(s))
	js.CopyBytesToJS(scratchU8, s)
	v, ok := scratchU8V[len(s)]
	if !ok {
		v = js.Global().Get("Uint8Array").New(scratchU8.Get("buffer"), 0, len(s))
		scratchU8V[len(s)] = v
	}
	return v
}
...
func ReadPixels(dst []byte, x, y, width, height int, format, ty Enum) {
	// Go slices can't be passed to JS directly (js.ValueOf panics - this
	// used to crash the engine's F12 screenshot). Read into a JS typed
	// array and copy back.
	if ty == Enum(UNSIGNED_BYTE) {
		buf := js.Global().Get("Uint8Array").New(len(dst))
		c.Call("readPixels", x, y, width, height, int(format), int(ty), buf)
		js.CopyBytesToGo(dst, buf)
	} else {
		buf := js.Global().Get("Float32Array").New(len(dst) / 4)
		c.Call("readPixels", x, y, width, height, int(format), int(ty), buf)
		u8 := js.Global().Get("Uint8Array").New(buf.Get("buffer"))
		js.CopyBytesToGo(dst, u8)
	}
}
