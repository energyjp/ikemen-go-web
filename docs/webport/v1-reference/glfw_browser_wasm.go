...
func CreateWindow(_, _ int, title string, monitor *Monitor, share *Window) (*Window, error) {
	...
	js.Global().Call("addEventListener", "resize", js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		// Deliberately no backing-store resize: the engine renders a fixed
		// GAME_W x GAME_H framebuffer (its cached viewport/scissor math
		// depends on it) and the page CSS-scales the finished canvas to fit
		// the window. Growing the backing here left the game rendered into
		// the bottom-left 640x480 of a larger black canvas after a resize.
		return nil
	}))
...
const (
	...
	KeyF24          Key = -iota - 2
	KeyF25          Key = -iota - 2
	// Numpad keys carry their JS keyCodes (96-111), which browsers deliver
	// directly; numpad Enter (keyCode 13 + location 3) is remapped to the
	// free code 108 in toKey.
	KeyKP0          Key = 96
	KeyKP1          Key = 97
	KeyKP2          Key = 98
	KeyKP3          Key = 99
	...
	KeyKP8          Key = 104
	KeyKP9          Key = 105
	KeyKPDecimal    Key = 110
	KeyKPDivide     Key = 111
	KeyKPMultiply   Key = 106
	KeyKPSubtract   Key = 109
	KeyKPAdd        Key = 107
	KeyKPEnter      Key = 108
...
func toKey(ke js.Value) Key {
	...
	switch {
	case key == 13 && ke.Get("location").Int() == KeyLocationNumpad:
		key = KeyKPEnter
...
// Browser Gamepad API bridge. navigator.getGamepads() is snapshotted once
// per frame in PollEvents (the engine queries buttons/axes per bound key,
// and a syscall/js round trip per query would be too slow).
//
// Pads with the W3C "standard" mapping are remapped to the GLFW/XInput
// button and axis order the engine's default JoystickConfig and per-name
// quirk handling (input.go JoystickState) were written against:
//   GLFW buttons: 0-3 A/B/X/Y, 4/5 LB/RB, 6 back, 7 start, 8/9 LS/RS,
//                 10-13 dpad U/R/D/L
//   GLFW axes:    0/1 left stick, 2/3 right stick, 4/5 LT/RT in -1..1
// Non-standard pads pass through raw; players remap in-game.
type gamepadState struct {
	present bool
	name    string
	buttons []Action
	axes    []float32
	// Resting axis values captured when the pad is first seen (non-standard
	// mappings only). Exotic pads rest hats/triggers at values like -1 or
	// 1.29; reporting deviation-from-rest instead of raw keeps a resting
	// axis from reading as permanently held input (which deadlocks the
	// options screen's binding scanner - it waits for an all-released
	// frame before accepting a press).
	axisRest []float32
	logged   bool
}

var gamepadStates [16]gamepadState
var gamepadOrder []js.Value // scratch for pollGamepads (avoids per-frame allocs)

// standard-mapping button index for each GLFW button index 0..13
var stdToGLFWButtons = [14]int{0, 1, 2, 3, 4, 5, 8, 9, 10, 11, 12, 15, 13, 14}
...
func pollGamepads() {
	...
	// The browser lists ANY HID device with buttons as a "gamepad" - USB
	// microphones, wheels, macro keypads. Rank real controllers first so
	// junk devices can't claim the P1 slot: standard-mapping pads, then
	// unrecognized pads that at least have axes, then button-only devices.
	...
	gamepadOrder = ordered
	slot := 0
	for i := 0; i < len(ordered) && slot < len(gamepadStates); i++ {
		p := ordered[i]
		g := &gamepadStates[slot]
		g.present = true
		g.name = p.Get("id").String()
		btns, axes := p.Get("buttons"), p.Get("axes")
		nb, na := btns.Length(), axes.Length()
		if !g.logged {
			g.logged = true
			js.Global().Get("console").Call("log",
				fmt.Sprintf("[gamepad %d] id=%q mapping=%q buttons=%d axes=%d",
					slot, g.name, p.Get("mapping").String(), nb, na))
		}
		if p.Get("mapping").String() == "standard" {
			g.buttons = g.buttons[:0]
			for _, si := range stdToGLFWButtons {
				act := Release
				if si < nb && btns.Index(si).Get("pressed").Bool() {
					act = Press
				}
				g.buttons = append(g.buttons, act)
			}
			g.axes = g.axes[:0]
			for a := 0; a < 4; a++ {
				v := float32(0)
				if a < na {
					v = float32(axes.Index(a).Float())
				}
				g.axes = append(g.axes, v)
			}
			// Analog triggers: standard buttons 6/7 -> axes 4/5, kept in
			// 0..1 with REST AT 0. The desktop XInput convention (rest -1)
			// breaks the options-screen scanner (getJoystickKey): its
			// generic branch treats any axis beyond +-0.2 as held input,
			// so a resting trigger at -1 reads as a stuck button and the
			// scanner never sees the "all released" state it needs before
			// accepting a press.
			for _, ti := range [2]int{6, 7} {
				v := float32(0)
				if ti < nb {
					v = float32(btns.Index(ti).Get("value").Float())
				}
				g.axes = append(g.axes, v)
			}
		} else {
			...
			for b := 0; b < nb; b++ {
				act := Release
				if btns.Index(b).Get("pressed").Bool() {
					act = Press
				}
				g.buttons = append(g.buttons, act)
			}
			if len(g.axisRest) != na {
				g.axisRest = g.axisRest[:0]
				for a := 0; a < na; a++ {
					g.axisRest = append(g.axisRest, float32(axes.Index(a).Float()))
				}
			}
			g.axes = g.axes[:0]
			for a := 0; a < na; a++ {
				g.axes = append(g.axes, float32(axes.Index(a).Float())-g.axisRest[a])
			}
		}
		slot++
	}
	for ; slot < len(gamepadStates); slot++ {
		if gamepadStates[slot].present {
			gamepadStates[slot] = gamepadState{}
		}
	}
}

func (joy Joystick) IsPresent() bool {
	return int(joy) >= 0 && int(joy) < len(gamepadStates) && gamepadStates[joy].present
}
...
func (joy Joystick) GetButtons() []Action {
	...
	return gamepadStates[joy].buttons
}
