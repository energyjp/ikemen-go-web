...
func Find(p string, src []byte, offset, limit int) (matches []*MatchData, err error) {
	...
	// Compiled-pattern cache: game scripts call string.match/gmatch with a
	// small set of recurring patterns every frame; recompiling them each
	// call was a dominant source of garbage on single-threaded wasm.
	cp, cached := patternCache[p]
	if !cached {
		pat := parsePattern(newScanner([]byte(p)), true)
		cp = compiledPattern{insts: compilePattern(pat), mustHead: pat.MustHead}
		if len(patternCache) >= 512 {
	...
	for sp := offset; sp <= len(src); {
		...
		if len(matches) == limit || cp.mustHead {
			break
		}
	}
	return
